...
const ChatWindow = ({ open, onClose, autoStartUserId = null }) => {
            ...
            if (existing) {
                ...
                setTimeout(async () => {
                    ...
                    if (newConv) {
                        ...
                        console.warn(`Could not find conversation with ${recipient.username} after sending.`);
                    }
                }, 500);
            }
        } catch (err) {
    ...
    // Auto-open chat when userId is passed
    useEffect(() => {
        const tryStartChat = async () => {
            ...
            try {
                ...
                if (username) {
                    await handleStartNewChatByUsername(username);
                } else {
                    console.warn('User has no username returned.');
                }
            } catch (err) {
            ...
            {view === 'list' ? (
                <Box p={1} flexGrow={1} overflow="auto">
                    <TextField
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                                handleStartNewChatByUsername(searchUsername);
                            }
                        }}
                    />
                    <Divider sx={{ my: 1 }} />
